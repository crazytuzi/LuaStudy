...
static TValue *index2addr (lua_State *L, int idx) {
  CallInfo *ci = L->ci;
  if (idx > 0) {
    TValue *o = ci->func + idx;  // func 指向改函数的“栈底”，然后线上递增idx个位置
    api_check(L, idx <= ci->top - (ci->func + 1), "unacceptable index");
    if (o >= L->top) return NONVALIDVALUE;
    else return o;
  }
  else if (!ispseudo(idx)) {  	/* 负数索引，位于栈索引的范围 */
    api_check(L, idx != 0 && -idx <= L->top - (ci->func + 1), "invalid index");
    return L->top + idx;
  }
  else if (idx == LUA_REGISTRYINDEX)	// 注册表 
    return &G(L)->l_registry;
  else {  								/* upvalues, 索引值比 LUA_REGISTRYINDEX 还要小 */
...
你不需要为结果分配空间： 其结果是一个 Lua 字符串，由 Lua 来关心其内存分配 
（同时通过垃圾收集来释放内存）。
这个转换非常的受限。 不支持符号、宽度、精度。 
转换符只支持 
'%%' （插入一个字符 '%'）， 
...
n 的最大值是 255 。
当 n 为零时， 这个函数将创建出一个 轻量 C 函数， 它就是一个指向 C 函数的指针。 
这种情况下，不可能抛出内存错误。
...
LUA_API void lua_pushcclosure (lua_State *L, lua_CFunction fn, int n) {
  lua_lock(L);
  if (n == 0) {					// 轻量级 C 函数
    setfvalue(L->top, fn);		// 直接把 C 函数指针压栈
    api_incr_top(L);
  }
  else {
    CClosure *cl;
    ...
    cl = luaF_newCclosure(L, n);// 创建 CClosure
    cl->f = fn;					// 设置C函数指针
    L->top -= n;				// 栈顶指针调整到上值下面的位置
    while (n--) {				// 把栈中的 upval 赋值给 CClosure 中的 upvalue[x]
      setobj2n(L, &cl->upvalue[n], L->top + n);
      /* does not need barrier because closure is white */
    }
    setclCvalue(L, L->top, cl);	// 把创建的 CClosuer 压栈
    api_incr_top(L);
    luaC_checkGC(L);
  }
  lua_unlock(L);
...
errfunc:
ctx: K函数上下文环境
k: K函数（为了在 lua 中 yield 之后，继续执行C中的代码）
*/
LUA_API int lua_pcallk (lua_State *L, int nargs, int nresults, int errfunc,
  ...
  c.func = L->top - (nargs+1);    /* 定位到栈中的函数所在索引。function to be called */
  if (k == NULL || L->nny > 0) {  /* no continuation or no yieldable? */
    c.nresults = nresults;  	  /* 期望闭包返回值的个数。do a 'conventional' protected call */
...
/*
reader: lua_load 用到的读取器函数， 每次它需要一块新的代码块的时候， 
lua_load 就调用读取器， 每次都会传入一个参数 data 。 
读取器需要返回含有新的代码块的一块内存的指针， 并把 size 设为这块内存的大小。
...
LUA_API const char *lua_setupvalue (lua_State *L, int funcindex, int n) {
  ...
  StkId fi;
  lua_lock(L);
  fi = index2addr(L, funcindex);  // 从闭包索引得到对象地址
  api_checknelems(L, 1);
